...
module N03
  ...
  ACTOR_POSITION = [[380, 250, 0], [440, 215, 0], [420, 300, 0], [475, 260, 0]]
  ...
  ACTION = {
    ...
    # Cell Rotate
    'Front Rotate' => ['motion', '', 0, 0, 0, 6, 10, false, true, ''],
    'Left Rotate' => ['motion', '', 1, 0, 0, 6, 10, false, true, ''],
    'Back Rotate' => ['motion', '', 3, 0, 0, 6, 10, false, true, ''],
    'Right Rotate' => ['motion', '', 2, 0, 0, 6, 10, false, true, ''],
    'Back Wpn Swing' => ['motion', '', 3, 0, 0, 2, 200, true, true, 'Vert Swing'],
    'Back Sec Swing' => ['motion', '', 3, 0, 0, 2, 50, true, true, 'Vert Swing L'],
    # Pause
    # -Action Name-      Type   Index   Vert  Horz  Pattern Speed  Z  Wait Shadow Weapon
    ...
    'Wpn Swing L' => ['motion', '', 1, 0, 1, 1, 300, false, true, 'Vert Swing L'],
    ...
    'Small Jump'              => ['move', 0, 0, 0, 0, -18, 0, [10, -10], 'Wpn Raised'],
    ...
    'Move to Center Curve'   => ['move', 4, 0, 0, 0, -20, 10, [20, -20], 'Move Left'],
    'Move to Center'         => ['move', 4, 0, 0, 0, 0, 0, [0, 0], ''],
    ...
    # Items
    'ItemShow' => ['wp', 0, [0, 9], [0, 0], 0, 360, 5, true, [2, 2], true, false, -1, '', '', false],
    # Armas Generales
    'Vert Swing' => ['wp', 0, [6, 0], [-4, -20], -45, 45, 4, false, [1, 1], true, false, -1, '', ''],
    # Escudo
    'Shield' => ['wp', 0, [0, 0], [0, -20], 0, -45, 0, false, [1, 1], true, true, -1, '', ''],
    # Segunda Arma
    'Vert Swing L' => ['wp', 0, [6, 0], [-4, -20], -45, 45, 4, false, [1, 1], true, true, -1, '', ''],
    # Armas de Impacto
    ...
    # -Action Name-              Type   anim hitAnim   i   n iPos nPos Speed EType Orbit   Z     Wait         Damage  Homing Camera Loop  NoFlip Weapon
    ...
    'BolaNieve'               => ['m_a', [137, 163], 0, 1, [0, 0], [4, -6], 10, 1, [3, 3], true, [true, false], true, true, true, false, false, ''],
    ...
    'Skill Anim LWT' => ['anime', -3, 1, false, false, true, true, false, false, true],
    ...
    # ● Proportion Change
    #--------------------------------------------------------------------------
    # This section here controls the proportion change of the background and the
    # battlers.
    # Target - Subject of the color change.
    #          [  0: Current Battler ]
    #          [  1: Selected Target  ]
    ...
    #          [  9: Everyone else than the target ]
    #          [ 10: All ]
    # Proportion - [ X Proportion, Y Proportion, Change Time, Return Time]
    #        Return Time - Time to return to the proportion after change is
    #                      done. 0 to not return to proportion.
    # Wait - Wait until change is done.
    # -Action Name-                         Type   Target   Proportion   Wait
    # In the ACTION hash, add these new entries:
    'Stretch X' => ['proportion', 0, [2.0, 1.0, 30, true]],  # Double width
    'Stretch Y' => ['proportion', 0, [1.0, 2.0, 30, true]],  # Double height
    'Low Squash Target' => ['proportion', 1, [1.25, 0.5, 60, true]],     # Squash effect
    'Squash Target' => ['proportion', 1, [1.5, 0.25, 60, true]],     # Squash effect
    'Low Squeeze Target' => ['proportion', 1, [0.5, 1.25, 60, true]],     # Squeeze effect
    'Squeeze Target' => ['proportion', 1, [0.25, 1.5, 60, true]],     # Squeeze effect
    'Grow' => ['proportion', 0, [1.5, 1.5, 30, true]],       # Scale up
    'Shrink' => ['proportion', 0, [0.5, 0.5, 30, true]],     # Scale down
    'Reset Proportion' => ['proportion', 0, [1.0, 1.0, 30, true]], # Reset to normal
    'Reset Target Proportion' => ['proportion', 1, [1.0, 1.0, 30, true]], # Reset to normal
    ...
    'Remove Hidden' => ['sta', 0, 0, '-', [40]],
    'Add Self Poison' => ['sta', 0, 0, '+', [2, 3, 4]],
    ...
    'Death Flag Movie' => %w[movie sample1],
    ...
    'Actor NOT1 Limit' => ['n_7', 0, '@battler.actor? && @battler.actor_id != 1'],
  ...
  FULLACTION = {
    ...
    'General Abnormal Stance' => ['Abnormal State/Generic', 'Generic Status Abnormal Tone', 'Wait', 'Wait',
                                  'Enemy Limit', '80'],
    'Standby' => %w[Wait 60],
    'Dead' => ['Fallen'],
    ...
    'Victory Meh Pose' => ['Erase Battle Anim', 'Fast Spin', 'Wpn Swing R', '120'],
    'Victory Small Pose' => ['Erase Battle Anim', 'Front Rotate', 'Small Jump', 'Small Jump', 'Small Jump',
                             'Small Jump', 'Small Jump', '120'],
    ...
    'Jump Secondary' => ['Small Jump', 'Swing Secondary'],
    'Swing Secondary' => ['Actor NOT1 Limit', 'Wpn Swing L', 'Actor ID1 Limit', 'Wpn Swing R', 'Skill Anim WT'],
    ...
    'Cut-in Branch' => ['Actor ID1 Limit', 'Cut-in A1', 'Actor ID2 Limit', 'Cut-in A2', 'Actor ID6 Limit', 'Cut-in A3',
                        'Actor ID7 Limit', 'Cut-in A4'],
    'Death Flag Video' => ['White Fade-In', '50', 'Death Flag Movie', 'White Fade-Out', '50'],
    'Space Background' => ['Zoom Out', "se('Blind',70)", 'Dim Background', 'All Members Except Self', 'Force Action 2',
                           'Transparent', 'Transparent', '40', 'Change Space Background', 'Hue Default'],
    'Space Background Release' => ['Reset Camera', 'Dim Background', "target2('Clear Transparent WT')",
                                   'Clear Transparent', '40', 'Remove Space Background', 'Hue Default'],
    ...
    'Skill Attack' => ['Normal Attack Start', 'Wait(Fixed)', 'Wpn Swing L', 'Skill Attack End'],
    ...
    'Generic Skill' => ['Skill Motion', 'Wpn Swing R', 'Damage Animation', 'Coordinate Reset Left'],

    'Generic Magic' => ['Magic Motion', 'Wpn Swing R', 'Damage Animation', 'Coordinate Reset'],

    'Depletion Magic' => ['Skill Motion', 'Wpn Swing R', 'Timed Charge Random Key', 'Damage Animation',
                          'Hutch MP Depletion', 'Coordinate Reset'],

    'Water Gun' => ['Skill Motion', 'Wpn Swing R', 'Water Gun Fire', 'Wait', 'Coordinate Reset'],

    'Throw Weapon' => ['Skill Motion', 'Wpn Swing R', '6', 'Wait(Fixed)', 'Wpn Throw Start', 'Wpn Throw Return',
                       'Coordinate Reset'],
    ...
    'Movie Attack' => ['Skill Motion', 'Wait(Fixed)', 'Death Flag Video', 'Normal Attack Start', 'Wpn Swing R',
                       'Normal Attack End'],

    'Wolf Transformation' => ['Skill Motion', 'anime(110,false)', 'Wolf Transform', 'Wait(Fixed)', '120',
                              'Coordinate Reset'],

    'Attack 5 Times' => ['Skill Motion', "Don't Collapse", 'Bow Exclude', 'Attack', 'Attack', 'Attack', 'Collapse',
                         'Coordinate Reset'],

    'Derived Skill' => ['Throw Weapon', 'Derive Multi-Stage Attack', 'Slash Attack'],

    'Background Change Attack' => ['Magic Motion', 'Wpn Swing R', 'Space Background', 'Damage Animation',
                                   'Space Background Release', 'Coordinate Reset'],

    'Picture Attack' => ['Magic Motion', 'Magic Square Display', 'Wpn Swing R', 'anime(80,false)', '60', 'Hard Shake',
                         'Skill Anim', 'Magic Square Erase', 'Coordinate Reset'],
    ...
    'GolpeFlama' => ['Skill Motion', "Don't Collapse",
                     ...
                     'Wpn Swing R', '20', 'Hue Default', 'Collapse', 'Coordinate Reset Left'],
    ...
    'Salto Pesado' => ['Skill Motion', 'One Step Before Jump', 'Before Jump Slam',
                       ...
                       'Skill Anim', 'Hard Shake', 'Squash Target', 'Force Action', 'Hard Shake', 'Pitching', '20',
                       'One Step After Jump','Squeeze Target', '5', 'Reset Target Proportion', 'Hard Shake', 'Coordinate Reset'],

    'Salto Elevado' => ['Skill Motion', 
                          'One Step Before Jump', 'Before Jump Slam', 'Timed Charge Bar AS',
                          ...
                          '5',
                          'Rotate Right Once',
                          'Move to Center Curve',
                          'Low Squeeze Target',
                          '5',
                          'Reset Target Proportion',
                          'Force Action', 
                          'One Step After Jump',
                          'Coordinate Reset'],
    ...
    'Placaje' => ["Don't Collapse", 'Skill Motion', 'Move to Enemy', "target('Damage Levantamiento')", 'Skill Anim', 'Medium Shake', '10',
                  'One Step After Jump', "target('Coordinate Reset Left')", '20', 'Coordinate Reset Left', '20', 'Collapse'],

    'NadoRapido' => ["Don't Collapse", 'Skill Motion', 'One Step After Jump', 'Timed Charge Bar XY', 'Rotar 90',
                     'SwiftNado', 'Move to Enemy', 'Rotar -90', 'One Step After Jump', 'One Step After Move', 'Collapse', 'Coordinate Reset Left'],

    'Protocolo Corrida' => ["Don't Collapse", 'Skill Motion', 'One Step After Jump', 'CorridaToro', 'Move to Enemy',
                            "target('Self Spin')", 'One Step After Jump', 'One Step After Move', 'Collapse', 'Coordinate Reset', "target('Coordinate Reset')"],
    ...
    'Ventisca' => ["Don't Collapse", 'Zoom Out', 'Wpn Swing R', 'Timed Charge Bar Rotate', 'Loop Start', 'Self Spin',
                   'If Timed Hit', 'Loop End', 'Skill Motion', 'Reset Camera', 'Damage Animation', 'Collapse', 'Coordinate Reset Left'],

    'Flujo Subterraneo' => ["Don't Collapse", 'Zoom Out', 'Wpn Swing R', 'Timed Charge Bar LR Loop', 'Loop Start',
                            'Simple Jump', 'If Timed Hit', 'Loop End', 'Skill Motion', 'Reset Camera', 'Damage Animation', 'Collapse', 'Coordinate Reset'],

    'Acuasphere' => ["Don't Collapse", 'Skill Motion', 'Wpn Swing R', 'Timed Charge Bar XY Loop', 'Loop Start',
                     'anime_me(146,false)', '20', 'If Timed Hit', 'Loop End', 'Wait', 'Acuaesfera Fire', 'Coordinate Reset', 'Collapse'],

    'Rope Bind' => ["Don't Collapse", 'Skill Motion', 'Move Front Enemy', 'Zoom Self',
                    'Timed Charge Stages Back', 'Fast Spin', 'Skill Anim', '15', 'Coordinate Reset Left', 'Reset Camera', 'Collapse'],

    'Boulder Throw' => ["Don't Collapse", 'Skill Motion', 'Timed Charge Bar UP', 'If Timed Hit', 'Fast Spin',
                        'anime_me(188,false)', '15', 'Rotate Right Once', 'Rock Fire', 'Coordinate Reset', 'Collapse'],

    'IceBlock Throw' => ["Don't Collapse", 'Skill Motion', 'Timed Charge Bar UP', 'If Timed Hit', 'Fast Spin',
                         'anime_me(191,false)', '55', 'Rotate Right Once', 'Ice Fire', 'Coordinate Reset', 'Collapse'],

    'Feather Damage' => ['Feather Fire', "Damage Animation"],

    'Dardo Pluma' => ["Don't Collapse", 'Move Front Enemy', 'One Step After Jump', 'Multistrike Attack', '5',
                      'Loop Start', 'If Indicator A', 'Feather Damage', 'If Timed Hit', 'Loop End', 'Collapse', 'Coordinate Reset'],

    'Destrozaladro' => ["Don't Collapse", 'Skill Motion', 'One Step After Jump', 'Timed Charge CCW Rotate',
                        'Loop Start', 'Fast Spin', 'If Timed Hit', 'Loop End', 'Rotar 90', 'SpinDrill', 'Move to Enemy', 'Rotar -90', 'One Step After Jump', 'One Step After Move', 'Collapse', 'Coordinate Reset']
