...
module YEA
  module COMMAND_PARTY
    ...
    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    COMMAND_TEXT   = "Formation"    # Text used for the command.
    PARTY_COOLDOWN = 3          # Turns that must pass between each change.
    SHOW_SWITCH    = 85          # If switch is on, show command. 0 to disable.
