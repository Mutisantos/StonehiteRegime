...
# ¥ Updates
# =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
# 2015.06.06 - Acomodado el código para que sea compatible con Tankentai sin que se
#              omita la animacion de victoria de los personajes ni sobrelapar
#              las ganancias y la subida de nivel.
...
class Window_VictoryLevelUp < Window_Base
  ...
  def draw_arrows
    ...
    change_color(system_color)
    for i in 0..8
      draw_text(dx, dy, 24, line_height, "->", 1) ##Error AQUI
      dy += line_height
    end
  end
...
class Scene_Battle < Scene_Base
  ...
  def show_victory_level_up(actor, temp_actor)
    ...
    @victory_spoils_window.hide ##Esconder para que no haya conflicto
    #---
