...
module YEA
  module MENU
    ...
    SHOW_FORMATION_SWITCH    = 85 # If true, the Formation menu is shown
    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    # - Main Menu Settings -
    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
...
class Window_MenuCommand < Window_Command
  ...
  def make_command_list
    for command in YEA::MENU::COMMANDS
      ...
      when :formation
        show = YEA::MENU::SHOW_FORMATION_SWITCH
        add_formation_command if $game_switches[show]
      when :save
        add_original_commands if $TEST || $BTEST
        add_save_command if $TEST || $BTEST
      when :game_end
