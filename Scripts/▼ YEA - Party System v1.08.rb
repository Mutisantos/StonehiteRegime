...
module YEA
  module PARTY
    ...
    COMMANDS_KEYS = [:change,:remove,:revert,:finish]
...
class Game_Party < Game_Unit
  ...
  def add_actor(actor_id)
    ...
    return if @battle_members_array.nil?
...
class Window_PartyMenuCommand < Window_Command
  ...
  def make_command_list
    for command in YEA::PARTY::COMMANDS_KEYS
      case command
      when :change, :remove, :revert
        add_command(YEA::PARTY::COMMANDS[command], command)
      when :finish
        add_command(YEA::PARTY::COMMANDS[command], command, enable_cancel?)
      else; next
      end
    end
  end
